...
import {
  ChevronLeft, ChevronRight, Home, Layers, Info,
  Phone, ShoppingCart, User, LogOut
} from 'lucide-react';
import { ShopContext } from '../context/ShopContext';
import { toast } from 'react-toastify';
import { FaShoppingCart, FaUser, FaSignInAlt, FaSignOutAlt, FaClipboardList } from 'react-icons/fa';

const Sidebar = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const [isOpen, setIsOpen] = useState(true);
  const { getTotalCartItems, token, setToken, userName, setUserName } = useContext(ShopContext);
  ...
  const handleLogout = () => {
    setToken(null);
    setUserName(''); // ✅ Xóa tên khi logout
  ...
  return (
    <div className={`fixed left-0 top-0 h-screen bg-gradient-to-b from-white via-indigo-100 to-indigo-200 text-gray-800 shadow-xl z-40 transition-all duration-300 ${isOpen ? 'w-72' : 'w-20'} overflow-hidden`}>
      ...
      {/* User Greeting */}
      {token && userName && isOpen && (
        <div className="px-4 pb-2 text-sm font-semibold text-indigo-800">
          👋 Xin chào, {userName}
        </div>
      )}
